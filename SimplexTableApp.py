...
with st.form("input_form"):
    ...
    g = st.number_input("g (coeff of x in Z)", value=1.0)
    h = st.number_input("h (coeff of y in Z)", value=1.0)
...
# Helper functions
def to_fraction_matrix(a, b, c, d, e, f, g, h, objective):
...
def calculate_ratios(table, pivot_col):
    ...
    for i in [1, 2]:
        ...
        if coeff > 0:
            table[i][7] = table[i][6] / coeff
            logs.append(f"Row {i}: ratio = {table[i][6]} / {coeff} = {table[i][7]}")
        else:
            table[i][7] = None
            logs.append(f"Row {i}: pivot column value <= 0, ratio skipped.")
    return logs

def find_pivot_column(table):
    return min(range(1, 3), key=lambda j: table[3][j])

def find_pivot_row(table, pivot_col):
    candidates = [(i, table[i][7]) for i in [1, 2] if table[i][7] is not None]
    if not candidates:
        return None
...
def pivot_operation(table, pivot_row, pivot_col):
    ...
    logs.append(f"Normalizing pivot at (Row {pivot_row}, Col {pivot_col}) = {pivot_val}")
    table[pivot_row][1:7] = [val / pivot_val for val in table[pivot_row][1:7]]
    logs.append(f"Row {pivot_row} after normalization: {table[pivot_row][1:7]}")
    for r in [1, 2, 3]:
        if r != pivot_row:
            factor = table[r][pivot_col]
            logs.append(f"Eliminating row {r}, factor = {factor}")
            new_vals = [table[r][j] - factor * table[pivot_row][j] for j in range(1, 7)]
...
def is_optimal(table):
    return all(val >= 0 for val in table[3][1:3])

def extract_solution(table, objective):
    sol = {"x": 0, "y": 0}
...
if submitted:
    ...
    while not is_optimal(table):
        ...
        st.session_state.logs.append(f"Pivot column: {pivot_col} ({table[0][pivot_col]})")
        ...
        if pivot_row is None:
            st.session_state.logs.append("Solution unbounded: no valid pivot row found.")
            break
...
if st.session_state.tables:
    ...
    for idx, tbl in enumerate(st.session_state.tables):
        st.subheader(f"Tableau {idx}")
        st.code(tabulate(tbl, tablefmt="fancy_grid"))

# Display final result
if st.session_state.final_vars:
    st.markdown("---")
    st.subheader("🎉 Final Result")
    st.write(f"x = {st.session_state.final_vars['x']}")
    st.write(f"y = {st.session_state.final_vars['y']}")
    st.write(f"Z = {st.session_state.final_vars['Z']}")
